...
public class Library
{
	...
	public void download(AudioContent content)
	{
		if(content.getType() == Song.TYPENAME){ //checking the type of given content
			...
			songs.add((Song)content);  //otherwise adding the content to songs arraylist by typecasting it to Song
		...
		if(content.getType() == AudioBook.TYPENAME){ //checking the type of given content
			for(int i=0;i<audiobooks.size();i++){   //checking if the audiobook is already in the audiobooks arraylist  
				if(audiobooks.get(i).equals(content)){
					throw new AlreadyDownloaded("Audiobook "+content.getTitle()+" already downloaded");  //if the audiobook is in the audiobooks arraylist then setting the errormsg
				}
			}
			audiobooks.add((AudioBook)content);  //otherwise adding the content to audiobooks arraylist by typecasting it to Audiobook
  ...
  // Print the name of all artists. 
	...
	public boolean deleteSong(int index)
	{
		...
		if (index < 1 || index > songs.size()){ //checking if the index is valid for songs arraylist	
			errorMsg = "Song Not Found"; //if not then setting the errormsg 
			return false;
		}
		songs.remove(index-1);  //otherwise removing the song from songs list 
		return true;
	}
  ...
  // Write a class SongLengthComparator that implements
	...
	public void playSong(int index)
	{
		if (index < 1 || index > songs.size()) //checking if the index is valid for songs arraylist
		{
			throw new NotFound("Song Not Found");  //if not then setting the errormsg 
		}
